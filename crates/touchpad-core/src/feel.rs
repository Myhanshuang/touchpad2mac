...
pub const fn feel_parameter_specs() -> &'static [FeelParameterSpec] {
    &[
        ...
        FeelParameterSpec { key: "scroll.momentum_tau_ms", group: "Scroll", unit: "ms", min: Some(50.0), max: Some(1200.0), step: Some(25.0), effect: "Compatibility-only in the current takeover path; touchpad-core ends finger scroll on release and does not synthesize kinetic continuation." },
        FeelParameterSpec { key: "scroll.momentum_start_speed_mm_per_s", group: "Scroll", unit: "mm/s", min: Some(10.0), max: Some(200.0), step: Some(5.0), effect: "Compatibility-only in the current takeover path; retained for stable settings migration." },
        FeelParameterSpec { key: "scroll.momentum_stop_speed_mm_per_s", group: "Scroll", unit: "mm/s", min: Some(1.0), max: Some(50.0), step: Some(1.0), effect: "Compatibility-only in the current takeover path; retained for stable settings migration." },
