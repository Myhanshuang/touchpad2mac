...
fn feel_ui_text(key: &str) -> Option<FeelUiText> {
    Some(match key {
        ...
        "scroll.momentum_tau_ms" => FeelUiText {
            label: "滚动惯性衰减时间（兼容保留）",
            controls: "兼容旧配置保留。当前 takeover 的 touchpad-core 在双指离板时立即发送 ScrollEnd，不再在输入层生成软件惯性；该参数当前不改变实时滚动行为。",
            lower: "当前实时路径无变化。若未来在桌面/应用上下文层重新启用 kinetic continuation，较小值代表更快衰减。",
            higher: "当前实时路径无变化。若未来在桌面/应用上下文层重新启用 kinetic continuation，较大值代表更长滑行。",
            tuning: "当前无需调节；保留它仅用于旧 settings.json 兼容，避免配置迁移时丢字段。",
        },
        "scroll.momentum_start_speed_mm_per_s" => FeelUiText {
            label: "惯性启动速度门槛（兼容保留）",
            controls: "兼容旧配置保留。当前 core 不在手指离板后启动 synthetic momentum，因此这个门槛不参与 takeover 实时决策。",
            lower: "当前实时路径无变化。旧 momentum 算法中，较低值会让较慢的松手也进入惯性。",
            higher: "当前实时路径无变化。旧 momentum 算法中，较高值只允许快速甩动启动惯性。",
            tuning: "当前无需调节；finger scroll 的生命周期由真实接触结束，kinetic continuation 应由有目标上下文的更高层负责。",
        },
        "scroll.momentum_stop_speed_mm_per_s" => FeelUiText {
            label: "惯性停止速度门槛（兼容保留）",
            controls: "兼容旧配置保留。当前 core 在 finger release 当帧结束 ScrollBegin/ScrollDelta 生命周期，不再等待软件惯性速度衰减。",
            lower: "当前实时路径无变化。旧 momentum 算法中，较低值意味着更长的低速尾巴。",
            higher: "当前实时路径无变化。旧 momentum 算法中，较高值意味着更早停止。",
            tuning: "当前无需调节；字段关系仍继续校验，以保证旧配置文件可稳定读写和以后迁移。",
        },
...
fn gesture_target_ui(name: &str) -> Option<GestureUiText> {
    Some(match name {
        ...
        "middle-click" => GestureUiText {
            label: "鼠标中键",
            description: "仅用于三指轻点；完整 tap 判定结束后发送一次中键按下+释放，不会与三指拖拽同时触发。",
        },
...
<div id="feel"></div><fieldset><legend>输入时禁用触控板（DWT）</legend><div id="dwt"></div></fieldset><fieldset><legend>手势绑定</legend><div id="gestures"></div></fieldset><p id="status" class="ok">已加载并验证当前设置。</p><button id="download">导出 settings.json</button><button id="reset">恢复载入时设置</button><details><summary>查看 JSON</summary><pre id="json"></pre></details>
...
function dwtUI(){{const root=document.getElementById('dwt');root.innerHTML='';const row=document.createElement('div');row.className='drag-toggle';const b=document.createElement('input');b.type='checkbox';b.checked=cfg.dwt.enabled;b.onchange=()=>{{cfg.dwt.enabled=b.checked;update();}};const label=document.createElement('label');label.append(b,document.createTextNode(' 输入时抑制新的触摸接触'));const note=document.createElement('div');note.className='gesture-desc';note.textContent='仅监听自动配对的内置键盘，evdev 只读打开且绝不 grab；修饰键不会触发。已提交的指针、滚动、手势和拖拽不会被后来的按键中断。';row.append(label,note);root.append(row);for(const item of [{{key:'short_timeout_ms',label:'首次按键抑制时间',min:1,max:5000}},{{key:'long_timeout_ms',label:'连续输入抑制时间',min:1,max:5000}}]){{const card=document.createElement('div');card.className='param';const title=document.createElement('div');title.innerHTML='<span class="param-title">'+item.label+'</span> &nbsp;<code class="key">dwt.'+item.key.replaceAll('_','-')+'</code>';const ctl=document.createElement('div');ctl.className='control';const q=document.createElement('input'),n=document.createElement('input');q.type='range';q.min=item.min;q.max=item.max;q.step=10;q.value=cfg.dwt[item.key];n.type='number';n.min=item.min;n.max=item.max;n.step=10;n.value=cfg.dwt[item.key];const setv=v=>{{const x=Math.round(Number(v));cfg.dwt[item.key]=x;q.value=x;n.value=x;update();}};q.oninput=()=>setv(q.value);n.onchange=()=>setv(n.value);ctl.append(q,n);card.append(title,ctl);root.append(card);}}}}
function gestureUI(){{const root=document.getElementById('gestures');root.innerHTML='';const drag=document.createElement('div');drag.className='drag-toggle';const db=document.createElement('input');db.type='checkbox';db.checked=cfg.gestures.three_finger_drag_enabled;db.onchange=()=>{{cfg.gestures.three_finger_drag_enabled=db.checked;update();}};const dl=document.createElement('label');dl.append(db,document.createTextNode(' 启用三指拖拽'));const dk=document.createElement('div');dk.innerHTML='<code class="key">gesture.three-finger-drag-enabled</code>';const dn=document.createElement('div');dn.className='gesture-desc';dn.textContent='开启后，三指平移优先由拖拽策略取得 ownership，三指 swipe 通常不可达；四指 swipe 不受影响。关闭后可重新把三指 swipe 用作桌面切换等动作。三指 tap candidate 仍可保留。';drag.append(dl,dk,dn);root.append(drag);for(const t of triggers){{const r=document.createElement('div');r.className='gesture';const left=document.createElement('div');const name=document.createElement('div');name.className='gesture-name';name.textContent=t.label;const key=document.createElement('code');key.className='key';key.textContent='gesture.'+t.name;const desc=document.createElement('div');desc.className='gesture-desc';desc.textContent=t.description;left.append(name,key,desc);const right=document.createElement('div');const s=document.createElement('select');const availableTargets=t.name==='three-finger-tap'?targets:targets.filter(x=>x.name!=='middle-click');for(const x of availableTargets){{const o=document.createElement('option');o.value=x.name;o.textContent=x.label+'  ('+x.name+')';s.append(o);}}s.value=cfg.gestures.bindings[t.name];const td=document.createElement('div');td.className='target-desc';const refresh=()=>{{const x=targets.find(x=>x.name===s.value);td.textContent=x?x.description:'';}};s.onchange=()=>{{cfg.gestures.bindings[t.name]=s.value;refresh();update();}};refresh();right.append(s,td);r.append(left,right);root.append(r);}}}}
function validate(){{const p=cfg.feel.pointer,s=cfg.feel.scroll,g=cfg.feel.gesture,d=cfg.feel.drag;if(p.max_gain<p.min_gain)return'pointer max_gain < min_gain';if(s.max_gain<s.min_gain)return'scroll max_gain < min_gain';if(s.axis_lock_release_ratio>=s.axis_lock_engage_ratio)return'axis lock release must be below engage';if(s.momentum_stop_speed_mm_per_s>=s.momentum_start_speed_mm_per_s)return'momentum stop must be below start';if(d.commit_threshold_mm>=g.multi_swipe_commit_mm)return'drag threshold must stay below multi-swipe threshold';if(cfg.dwt.short_timeout_ms<1||cfg.dwt.short_timeout_ms>5000)return'DWT short timeout outside 1..5000 ms';if(cfg.dwt.long_timeout_ms<1||cfg.dwt.long_timeout_ms>5000)return'DWT long timeout outside 1..5000 ms';if(cfg.dwt.short_timeout_ms>cfg.dwt.long_timeout_ms)return'DWT short timeout must be <= long timeout';return'';}}
function update(){{const e=validate(),st=document.getElementById('status');st.textContent=e||'当前参数满足编辑器约束。';st.className=e?'bad':'ok';document.getElementById('download').disabled=!!e;document.getElementById('json').textContent=JSON.stringify(cfg,null,2);}}function render(){{feelUI();dwtUI();gestureUI();update();}}document.getElementById('download').onclick=()=>{{const b=new Blob([JSON.stringify(cfg,null,2)+'\n'],{{type:'application/json'}}),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download='settings.json';a.click();URL.revokeObjectURL(u);}};document.getElementById('reset').onclick=()=>{{cfg=structuredClone(initial);render();}};render();</script></body></html>"#
    )
}
